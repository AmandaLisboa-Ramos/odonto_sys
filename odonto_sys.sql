-- todos os create table foram criados por Kayo
...
--Alexandre
insert into consultas (paciente, dentista, data_consulta, hora_consulta, descricao_consulta, prescricao_consulta, status)
values
...
--Create 1 Simone
CREATE INDEX idx_dataConsulta_dentista ON consultas (dentista, paciente, data_consulta);

--Create 2 Simone
CREATE INDEX idx_consultas_procedimentos ON consultas_procedimentos (procedimento);
...
--Cancelar uma consulta (se respeitar o prazo) - Amanda
UPDATE consultas
SET status = 'cancelada'
WHERE id_consulta = 10
AND data_consulta > current_date + INTERVAL '2 days';

--Delete 1: Remover os “cancelados” da lista consulta - Alexandre
DELETE FROM consultas_procedimentos
WHERE consulta IN (
  SELECT id_consulta FROM consultas WHERE status = 'cancelada'
);
DELETE FROM consultas
WHERE status = 'cancelada';
select * from consultas;

--Delete 2: Deletar os horarios de domingo - Simone
delete from horarios 
where diaSemana_horario = '1';
select * from horarios;

--Delete 3: Deletar o login do brunogargia - Kayo
delete from atendentes
where login_atendente = 'brunogarcia';
select * from atendentes;

select* from consultas;

--Quantidade de consultas por especialidade: Amanda
select especialidade_dentista, count(id_consulta) as "Total de consultas"
from dentistas
join consultas
on id_dentista = dentista
group by especialidade_dentista;

--Quantidade de consultas por dentista, em ordem decrescente: Kayo
select nomeCompleto_dentista, count(id_consulta)
from dentistas
join consultas on id_dentista = dentista
group by nomeCompleto_dentista
order by count(id_consulta) desc;

--Pacientes com maior número de consultas: Amanda
...
--Média de consultas por dentista: Simone
select
avg(consultas)::decimal(10,2) AS media_consultas
from (select
count(*) AS consultas
from consultas
group by dentista) as temp;